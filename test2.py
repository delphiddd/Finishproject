...
class Linkedlist():
    ...
    def mergeSort(self , h):
        if h is None or h.next is None:
            return h
        mid = self.getmid(h)#ชี้ที่โหนดกลาง
        next_mid = mid.next#ชี้ที่โหนดถัดไป
        
        mid.next = None#ตัดโนหดกันทำให้โหนดกลางเเบ่งกัน

        left = self.mergeSort(h)#ทำการmergฝั่งซ้าย
        right = self.mergeSort(next_mid)#ทำการmergฝั่งขวา
        
        sortedd = self.merge(left , right)#นำโหนดซ้ายเเละขวาเข้าไปmerg
        return sortedd
    def getmid(self , h):
        if h == None:
            return h
    ...
    def print_linkedlist(self):
        ...
        while curr != None:
            ...
            curr = curr.next
        print(strings)
    def seraching(self , key):
        curr = self.head
        while curr != None:
            if curr.key == key:
                print("Found..key => " , curr.key , "=> " , curr.point)
                break
            curr = curr.next
        else:
            print("NOT..Found")
